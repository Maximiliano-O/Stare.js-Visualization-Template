...
function chart(querySelector, data, opts) {
  ...
  documents.map(d => {
    ...
    for (const [word, positions] of Object.entries(documentWords)) {
      const splittedPositions = splitIntoEqualSegments(
        positions,
        documentLength,
        finalOptions.breakLine
      );
      let currentX = 1;

      // Print cubes representing the frequency of a keyword/term.
      for (let index = 0; index < splittedPositions.length; index++) {
        const frequency =
          splittedPositions[index].length > 8
            ? 8
            : splittedPositions[index].length;
