...
const defaultOptions = {
  ...
  tooltip: {
    style:
      "position: absolute;padding: 4px; width: 300px; background-color: #f9f9f9; border: 1px solid; border-radius: 2px; pointer-events: none; opacity: 0;",
    content: (item) => {
      return `<div>Ranking: <b>${get(
        item,
        "metrics.ranking",
        null
      )}</b></div><div><b>${item.title}</b></div><div>${item.snippet}</div>`;
    },
    onClick: (item) => {
...
function createTooltip({ tooltip: tooltipConfig }) {
  tooltip = select("body").append("div").attr("style", tooltipConfig.style);
}

function showTooltip(d, event, { tooltip: tooltipConfig }) {
...
function processData(data) {
  ...
  for (let i = data.length - 1; i >= 0; i--) {
    if (isArray(data[i].metrics.links)) {
      ...
      const ifExistsIndex = formattedData.nodes.findIndex(
        (n) => n.id === data[i].metrics.links[0]
      );
      if (ifExistsIndex === -1) {
      ...
      for (let j = data[i].metrics.links.length - 1; j > 0; j--) {
        ...
        if (
          formattedData.nodes.findIndex((n) => n.id === currentHostname) === -1
        ) {
...
function chart(querySelector, data, opts) {
  ...
  const svg = select(querySelector)
    .attr("viewBox", [
      -finalOptions.width / 2,
      -finalOptions.height / 2,
      finalOptions.width,
      finalOptions.height,
    ])
  ...
  const node = svg
    ...
    .on("mousemove", function(event, d) {
      if (get(formattedData.nodes[d.index], "type") === "mainNode") {
        tooltip.style("left", `${event.pageX}px`).style("top", `${event.pageY}px`);
      }
    })
    .on("mouseout", (n) => {
    ...
    .attr("r", (n) => {
      if (get(formattedData.nodes[n.index], "type") === "mainNode") {
        ...
        return typeof radius === "function"
          ? radius(formattedData.nodes[n.index], data)
          : radius;
      }
