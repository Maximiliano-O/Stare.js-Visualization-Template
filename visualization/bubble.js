...
import { select, scaleOrdinal, schemeCategory10, format as _format, pack as _pack, hierarchy } from "d3";
...
function showTooltip(d, options, event) {
  let html = options.tooltip.content(d);
...
function chart(querySelector, data, opts) {
  ...
  const node = svg.selectAll("g")
    ...
    // Updated event handlers with explicit event parameter
    .on("mouseover", (event, n) => showTooltip(n.data, finalOptions, event))
    .on("mousemove", (event) => moveTooltip(event))
    .on("mouseout", () => hideTooltip())
    .on("click", (event, n) => finalOptions.tooltip.onClick(n.data));
